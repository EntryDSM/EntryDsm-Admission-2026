...
import { monitoringQueryKeys, getMyAccount } from "../apis";

/**
 * 내 계정(권한) 조회 훅. 모니터링 접근 가드 전용이라 캐시를 신선하게 유지하지 않고(staleTime 0),
 * 재시도 없이 즉시 성공/실패를 판정해 차단 지연을 줄인다.
 * 재확인은 라우트 이동 시의 invalidate 로만 일어나도록 포커스/재접속 자동 refetch 는 끈다.
 */
export const useMyAccount = () => {
  const query = useQuery({
    queryKey: monitoringQueryKeys.account.me,
  ...
  return {
    ...
    isCheckingAccount: query.isPending || query.isFetching,
