  ...
  └─ GitHub Actions
       ...
       └─ deploy job: artifact 다운로드 → aws s3 sync → CloudFront 무효화 + smoke test
            └─ S3 단일 버킷 (ap-northeast-2, 비공개)  ← OAC ── CloudFront (ACM, SPA rewrite)
                 └─ dsm-Entry/frontend/{app} 폴더로 구분 │ 배포별 Origin path = /폴더
                                                                   ↑
                                                 Cloudflare DNS (CNAME, 회색 구름 = DNS 전용)
```

| 앱               | 도메인                     | 버킷 폴더                             | CloudFront ID (기입) |
| ---------------- | -------------------------- | ------------------------------------- | -------------------- |
| entry-user       | `entrydsm.hs.kr`           | `dsm-Entry/frontend/entry-user`       |                      |
| entry-auth       | `auth.entrydsm.hs.kr`      | `dsm-Entry/frontend/entry-auth`       |                      |
| entry-admin      | `admin.entrydsm.hs.kr`     | `dsm-Entry/frontend/entry-admin`      |                      |
| entry-admission  | `admission.entrydsm.hs.kr` | `dsm-Entry/frontend/entry-admission`  |                      |
| entry-monitoring | `monitor.entrydsm.hs.kr`   | `dsm-Entry/frontend/entry-monitoring` |                      |

smoke test가 이 규칙(`{sub}.entrydsm.hs.kr`)으로 도메인을 계산하므로 임의로 바꾸면 배포 검증이 실패한다.

- **S3 버킷은 1개를 전 앱이 공유**하고 폴더(prefix)로 구분한다: `dsm-Entry/frontend/{앱 이름}` (총 5개 폴더). 폴더 이름은 워크플로우가 이 규칙으로 자동 계산하므로 임의로 바꿀 수 없고, 배포 시 자동 생성되므로 미리 만들 필요도 없다.
- CloudFront 배포는 도메인마다 1개씩 **총 5개**. 각 배포의 **Origin path**가 자기 폴더를 가리킨다(4단계).
...
2. 도메인 이름에 2개를 추가한다. 서브도메인이 전부 한 단계 아래이므로 와일드카드 하나로 전부 커버된다.
   - `entrydsm.hs.kr`
   - `*.entrydsm.hs.kr`
...
이미 만들어 둔 **버킷 1개**를 전 앱이 공유한다. 아래 상태인지 확인한다. OAC 방식은 웹사이트 엔드포인트를 쓰지 않으므로 **버킷 이름이 도메인과 일치할 필요는 없다.**
...
## 4. CloudFront 배포 5개 생성

배포마다 동일한 절차를 반복한다 (앱별 1개, 총 5개).
...
| **Origin path** | 이 배포가 서비스할 폴더. 슬래시로 시작, 끝 슬래시 없음. 예: user 배포는 `/dsm-Entry/frontend/entry-user` (1단계 표의 폴더와 일치)    |
| Origin access   | **Origin access control settings (OAC)** 선택 → 첫 배포에서 `Create new OAC` 생성, **나머지 4개 배포는 같은 OAC를 재사용**           |

Origin path를 설정하면 뷰어의 `/index.html` 요청이 S3 키 `dsm-Entry/frontend/entry-user/index.html`로 매핑된다. 배포마다 폴더가 다르므로 **5개 배포 전부 Origin path를 정확히 입력하는 것이 핵심**이다.

버킷 정책은 공유 버킷에 **한 번만** 설정하되, 5개 배포 ARN을 전부 나열한다 (콘솔이 배포 생성 시 보여주는 정책은 배포 1개짜리이므로 그대로 붙여넣지 말고 아래처럼 합친다. `<>` 부분을 실제 값으로 교체):
  ...
  "Statement": [
    {
      ...
      "Condition": {
        "StringEquals": {
          "AWS:SourceArn": [
            "arn:aws:cloudfront::<계정ID>:distribution/<user-배포ID>",
            "arn:aws:cloudfront::<계정ID>:distribution/<auth-배포ID>",
            "arn:aws:cloudfront::<계정ID>:distribution/<admin-배포ID>",
            "arn:aws:cloudfront::<계정ID>:distribution/<admission-배포ID>",
            "arn:aws:cloudfront::<계정ID>:distribution/<monitoring-배포ID>"
          ]
        }
      }
    }
...
| Alternate domain name (CNAME) | 해당 배포의 도메인 (예: `admin.entrydsm.hs.kr`)                                     |
...
1. CloudFront 콘솔 → Functions → Create function → 이름 `spa-rewrite` (계정당 1개 만들어 5개 배포가 공유)
2. 코드 입력:
...
4. 배포 5개 각각: 기본 동작 편집 → Function associations → **Viewer request**에 `spa-rewrite` 연결
...
Cloudflare → **entrydsm.hs.kr zone** → DNS → 레코드 5개 추가. **전부 회색 구름(DNS only).**

| Type  | 이름                        | 대상                               | 프록시 상태 |
| ----- | --------------------------- | ---------------------------------- | ----------- |
| CNAME | `entrydsm.hs.kr` (또는 `@`) | user 배포의 `dxxxx.cloudfront.net` | DNS only    |
| CNAME | `auth`                      | auth 배포                          | DNS only    |
| CNAME | `admin`                     | admin 배포                         | DNS only    |
| CNAME | `admission`                 | admission 배포                     | DNS only    |
| CNAME | `monitor`                   | monitoring 배포                    | DNS only    |
...
키가 유출·오용되거나 워크플로우가 변조돼도 피해가 배포 리소스에 국한되도록, S3의 배포 prefix와 CloudFront 배포 5개까지만 접근하도록 제한한다.

1. IAM → 사용자 → 생성: `github-actions-entry-deploy-prod`
2. 사용자에 인라인 정책으로 아래 최소 권한 부여 (`<>`를 실제 버킷명·계정ID·배포ID로 교체)

`s3:ListBucket`은 `s3:prefix` 조건으로 앱 폴더만 조회 가능하게 제한하고, 쓰기·무효화도 배포 리소스만 나열한다:
...
- 보안 자격 증명 → **액세스 키 생성**(용도: 서드파티 서비스) → 키 값을 7단계에서 `prod` GitHub **environment secrets**에 넣는다.
...
저장소 → Settings → Environments → `prod` 생성.

- **Deployment branches**: `main`만 허용으로 제한한다 (워크플로우의 브랜치 가드와 이중 보호).
  워크플로우가 임의 SHA 체크아웃을 허용하지 않으므로 이 정책이 실제로 배포되는 코드까지 보호한다.
- (선택) Required reviewers를 걸면 프로덕션 배포 전 승인 단계가 생긴다.
...
AWS 액세스 키는 공통 Secrets에 넣지 않는다 — 배포 전용 사용자 키이므로 7-3의 environment secrets로 관리한다.

### 7-3. `prod` Environment Secrets

Settings → Environments → `prod` → **Environment secrets**. deploy job이 `environment:`를 참조하므로 같은 이름의 environment secret이 자동으로 적용된다 (워크플로우 수정 불필요).

배포 ID는 비밀은 아니지만 **secret으로 넣어야 퍼블릭 Actions 로그에서 자동 마스킹**된다.
폴더 경로는 워크플로우가 계산하므로 별도 설정이 없다 (`dsm-Entry/frontend/{앱}`).

| 이름                          | 값                                |
| ----------------------------- | --------------------------------- |
| `AWS_ACCESS_KEY_ID`           | 6단계에서 발급한 배포 사용자의 키 |
| `AWS_SECRET_ACCESS_KEY`       | 6단계에서 발급한 배포 사용자의 키 |
| `CF_DIST_ID_ENTRY_USER`       | user 배포 ID                      |
| `CF_DIST_ID_ENTRY_AUTH`       | auth 배포 ID                      |
| `CF_DIST_ID_ENTRY_ADMIN`      | admin 배포 ID                     |
| `CF_DIST_ID_ENTRY_ADMISSION`  | admission 배포 ID                 |
| `CF_DIST_ID_ENTRY_MONITORING` | monitoring 배포 ID                |

### 7-4. `prod` Environment Variables
...
| `VITE_AWS_CONSOLE_URL`     | AWS 콘솔 로그인 URL                | **전 앱** 빌드에 필수, 실제 사용은 monitoring 헤더 |
...
- **트리거**: `main` 푸시 → prod 배포. 문서(`*.md`, `docs/`)만 바뀐 푸시는 스킵.
- **빌드/배포 job 분리 (공급망 방어)**:
...
- **동시성**: 연속 푸시하면 이전 배포가 끝날 때까지 대기(중간에 끊어 반쪽 배포가 되는 것 방지).
...
- Actions 탭의 **Run workflow**(workflow_dispatch)는 `main` HEAD 재배포용이다 (GitHub 변수 수정 후 재배포 등). 워크플로우의 브랜치 가드 때문에 **main에서만 동작**하며, 다른 브랜치를 선택해 실행하면 job이 스킵된다.
...
4. [ ] CloudFront 배포 5개 생성 — **배포별 Origin path**, 공유 버킷 정책(SourceArn 5개), `/assets/*` 동작 분리, 기본 동작에 함수 연결. **오류 응답 매핑은 만들지 않음**
5. [ ] Cloudflare CNAME 5개 (전부 회색 구름)
6. [ ] IAM 사용자·정책(`GetInvalidation` 포함)·액세스 키
7. [ ] GitHub Environment(prod) + 저장소 Secret 1개 + 환경 Secrets 7개 + Variables 8개
8. [ ] `main` 병합 → 5개 배포 잡 녹색 + smoke test 통과 → 도메인 수동 확인 + `git revert` 롤백 리허설 1회
